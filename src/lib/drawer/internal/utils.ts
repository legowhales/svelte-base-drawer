...
/**
 * Whether a click was generated by the keyboard or assistive technology rather
 * than a physical pointer (port of floating-ui's `isVirtualClick`).
 */
export function isVirtualClick(event: MouseEvent | PointerEvent): boolean {
	// Firefox reports AT-generated clicks with mozInputSource === 0.
	if ((event as { mozInputSource?: number }).mozInputSource === 0 && event.isTrusted) {
		return true;
	}

	if (/Android/i.test(navigator.userAgent) && 'pointerType' in event && event.pointerType) {
		// Android TalkBack dispatches clicks with detail 1.
